...
export function addManualTransaction(input) {
  ...
  if (typeof input.category !== 'string' || !db.prepare('SELECT name FROM categories WHERE user_id=? AND name=?').get(userId(),input.category)) throw new Error('Choose a category.');
